<template>
    <div class="meal-kits-container container mt-4">
      ...
      <div class="meal-kits-grid row">
        <div v-for="mealKit in mealKits" :key="mealKit.meal_kit_id" class="meal-kit-card col-md-4 mb-4">
          <div class="card h-100">
            <img :src="mealKit.image_url" class="meal-kit-image card-img-top" alt="Meal Kit Image">
            <div class="meal-kit-card-body card-body">
              <h5 class="meal-kit-title card-title">{{ mealKit.meal_name }}</h5>
              <p class="meal-kit-description card-text">{{ mealKit.description.substring(0, 100) }}...</p>
              <p><strong>Price:</strong> R{{ mealKit.price }}</p>
              <span v-if="mealKit.stock_quantity === 0" class="meal-kit-out-stock badge bg-danger">Out of Stock</span>
              <button class="meal-kit-info-btn btn btn-info w-100 mt-2" @click="viewMealKit(mealKit)">View More Info</button>
              <button v-if="mealKit.stock_quantity > 0" class="meal-kit-add-cart-btn btn btn-primary w-100" @click="addToCart(mealKit.meal_kit_id)">Add to Cart</button>
            </div>
          </div>
        </div>
      </div>
        ...
        <div class="modal-dialog modal-lg" role="document">
          <div class="modal-content">
            <div class="modal-header">
              <h5 class="modal-title">{{ selectedMealKit.meal_name }}</h5>
              <button type="button" class="btn-close" @click="selectedMealKit = null"></button>
            </div>
            <div class="modal-body">
              <img :src="selectedMealKit.image_url" class="meal-kit-detail-image img-fluid mb-3" alt="Meal Kit Image">
            ...
            <div class="modal-footer">
              <button type="button" class="meal-kit-close-btn btn btn-secondary" @click="selectedMealKit = null">Close</button>
            </div>
          </div>
        </div>
      </div>
